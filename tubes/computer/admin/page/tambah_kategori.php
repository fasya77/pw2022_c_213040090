	<div class="container">
		...
		<div class="kotak_form">
			...
			if (@$_GET['proses'] == "form_edit") {
				...
				$tampildata = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM kategori_produk WHERE id_kategori='$idkategori'"));
			} else {
			...
			<form method="post" action="<?php echo $action; ?>">
				...
				<div class="col-8">
					<input class="form_input" type="text" name="kategori" value="<?php echo @$tampildata['kategori']; ?>" placeholder="Kategori Produk" class="form_input">
				</div>
		...
		<div class="kotak_form">
			...
			$idkategori = mysqli_real_escape_string($conn, @$_GET['id_kategori']);
			if ($proses == "tambah") {
				$simpan = mysqli_query($conn, "INSERT INTO kategori_produk(kategori) VALUES('$kategori')");
				if ($simpan) {
			...
			if ($proses == "edit") {
				$update = mysqli_query($conn, "UPDATE kategori_produk SET kategori='$kategori' WHERE id_kategori='$idkategori'");
				if ($update) {
			...
			if ($proses == "hapus") {
				$hapus = mysqli_query($conn, "DELETE FROM kategori_produk WHERE id_kategori='$idkategori'");
				if ($hapus) {
			...
			<table border="1" cellpadding="3" cellspacing="1" width="100%">
				...
				if (isset($_POST['cari'])) {
					$kategori = mysqli_query($conn, "SELECT * FROM kategori_produk WHERE kategori LIKE '%" . $_POST['cari'] . "%'");
				}
